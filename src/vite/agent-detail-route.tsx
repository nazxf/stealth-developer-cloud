...
import { Bot, CheckCircle2, Clock3, GitBranch, LoaderCircle, Play, Square, XCircle } from "lucide-react";
...
export default function AgentDetailRoute() {
  ...
  async function startRun(event: FormEvent<HTMLFormElement>) {
    ...
    if (!normalizedPrompt || runPending) return;
    setRunPending(true);
    setError("");
    try {
      const response = await browserAPI.createAgentRun(agentId, { prompt: normalizedPrompt });
      setPrompt("");
      setSelectedRunID(response.run.id);
      await queryClient.invalidateQueries({ queryKey: ["agent-runs", agentId] });
    } catch (requestError) {
      setError(requestError instanceof BrowserAPIError ? requestError.message : "The agent run could not be created.");
    } finally {
      setRunPending(false);
    }
  }
