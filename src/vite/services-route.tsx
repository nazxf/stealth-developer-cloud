...
export default function ServicesRoute() {
  ...
  const services: ServiceCard[] = [
    ...(resourceQueries[0].data?.functions ?? []).map((item) => ({ id: item.id, kind: "function" as const, name: item.name, status: item.status, detail: item.runtime, href: `/projects/${encodeURIComponent(projectId)}/functions` })),
    ...(resourceQueries[1].data?.sites ?? []).map((item) => ({ id: item.id, kind: "site" as const, name: item.name, status: item.status, detail: item.framework, href: `/projects/${encodeURIComponent(projectId)}/sites` })),
    ...(resourceQueries[2].data?.databases ?? []).map((item) => ({ id: item.id, kind: "database" as const, name: item.name, status: "managed", detail: "PostgreSQL-compatible", href: `/projects/${encodeURIComponent(projectId)}/databases` })),
    ...(resourceQueries[3].data?.buckets ?? []).map((item) => ({ id: item.id, kind: "storage" as const, name: item.name, status: "managed", detail: `${item.used_bytes.toLocaleString()} bytes used`, href: `/projects/${encodeURIComponent(projectId)}/storage` })),
  ];
  const loading = projectQuery.isPending || resourceQueries.some((query) => query.isPending);
  const error = projectQuery.error ?? resourceQueries.find((query) => query.error)?.error;
