...
function AdminSettingsPanel({ organizations }: { organizations: BrowserOrganization[] }) {
  ...
  const planQuery = useQuery({ queryKey: queryKeys.organizationPlan(selectedOrganization?.id ?? ""), queryFn: () => browserAPI.organizationPlan(selectedOrganization!.id), enabled: Boolean(selectedOrganization) });
